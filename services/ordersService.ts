...
import { CollectionOrder, createOrder } from "../models/order";
...
export const ordersService = {
   ...
   */
  async listByCitizen(citizenId: string): Promise<CollectionOrder[]> {
   ...
   */
  async listByCollector(collectorId: string): Promise<CollectionOrder[]> {
    const { data, error } = await supabase
      .from(TABLES.ORDERS)
      .select(ORDER_SELECT)
   ...
   * @param {string} collectorId
   */
  async listForCollector(collectorId: string): Promise<CollectionOrder[]> {
    const { data, error } = await supabase
      .from(TABLES.ORDERS)
      .select(ORDER_SELECT)
   ...
   */
  async updateByCollector(orderId: string, collectorId: string, payload: OrderPayload) {
